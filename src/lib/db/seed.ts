...
async function seed() {
  ...
  try {
    ...
    const existingMuscleGroupNames = new Set(
      existingMuscleGroups.map((g) => g.name)
    );
    ...
    const muscleGroupMap = new Map(
      allMuscleGroups.map((group) => [group.name, group.id])
    );
    ...
    if (newRelationships.length > 0) {
      console.log(
        `Inserting ${newRelationships.length} new exercise-muscle group relationships...`
      );
      await db
